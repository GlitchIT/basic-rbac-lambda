...
const gets3Obj = async (objKey, isJson = false) => {
  const bconfig = await s3.getObject({
 ...
 *
 */
const puts3Obj = async (data, {key, filename = new Date().toISOString(), path = 'media/'}) => {
...
const getConfig = async () => {
  ...
  const config = await gets3Obj('config.json', true).catch(err => {
  ...
  let settings = await gets3Obj(config.settingsKey, true).catch(err => {
